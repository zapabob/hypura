...
use crate::profiler::types::{effective_secondary_storage_peak_bw, HardwareProfile};
...
fn compute_tier_capacities(
    ...
    // Single-channel model: one effective SSD read bandwidth (see `effective_secondary_storage_peak_bw`).
    let nvme_peak_bw = effective_secondary_storage_peak_bw(hw);
...
fn quick_estimate(
    ...
    let gpu_bw = hw
        ...
        .map_or(1e9, |g| g.bandwidth_bytes_per_sec as f64);
    let ram_bw = hw.memory.bandwidth_bytes_per_sec as f64;
    let nvme_bw = effective_secondary_storage_peak_bw(hw) as f64;
