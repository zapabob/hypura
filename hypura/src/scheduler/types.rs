...
/// Logical index for a future **secondary-storage pool** (separate volume / GGUF shard).
///
/// Today all NVMe-tier tensors stream from the **single** model path opened by `IoPool`.
/// Non-zero IDs are reserved for multi-volume placement once scheduling lands.
pub type SecondaryStoragePoolId = u8;

/// Default pool: single GGUF file on one filesystem path.
pub const DEFAULT_SECONDARY_STORAGE_POOL: SecondaryStoragePoolId = 0;
...
pub enum StorageTier {
    ...
    /// System RAM
    Ram,
    /// Slowest logical tier (typically flash SSD: NVMe or SATA). Not a bus-specific guarantee.
    ///
    /// Physical **multi-drive sharding** is not scheduled yet; see [`PrefetchOp::storage_pool_id`].
    Nvme,
}
...
pub struct PrefetchOp {
    ...
    /// Which secondary-storage pool to read from (future multi-volume layouts).
    #[serde(default)]
    pub storage_pool_id: SecondaryStoragePoolId,
}
