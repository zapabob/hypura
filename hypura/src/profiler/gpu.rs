...
fn profile_gpu_cuda_or_cpu() -> anyhow::Result<Option<GpuProfile>> {
    ...
    if let Some((name, vram_bytes)) = gpu {
        let (bandwidth, tflops) =
            lookup_nvidia_gpu(&name).unwrap_or_else(|| estimate_nvidia_gpu(vram_bytes));
...
const NVIDIA_SPECS: &[NvidiaSpec] = &[
    ...
    NvidiaSpec {
        pattern: "RTX 4080 Super",
        bandwidth_gb_s: 736.0,
        fp16_tflops: 103.9,
    },
    NvidiaSpec {
        pattern: "RTX 4080",
    ...
    NvidiaSpec {
        pattern: "RTX 3080 Ti",
        bandwidth_gb_s: 912.0,
        fp16_tflops: 65.0,
    },
    NvidiaSpec {
        pattern: "RTX 3080 12GB",
        bandwidth_gb_s: 912.0,
        fp16_tflops: 60.0,
    },
    NvidiaSpec {
    ...
    NvidiaSpec {
        pattern: "RTX 2080 Super",
        bandwidth_gb_s: 496.0,
        fp16_tflops: 43.6,
    },
    NvidiaSpec {
        pattern: "RTX 2080",
...
fn lookup_nvidia_gpu(name: &str) -> Option<(u64, f64)> {
    NVIDIA_SPECS
        .iter()
        .find(|s| name.contains(s.pattern))
        .map(|s| ((s.bandwidth_gb_s * 1e9) as u64, s.fp16_tflops))
}
