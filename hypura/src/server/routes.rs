...
pub struct AppState {
    ...
    /// Directories scanned for `.gguf` files (Kobold-lite model list).
...
fn collect_gguf_models(model_dirs: &[PathBuf], active_path: &str) -> anyhow::Result<Vec<AvailableModelItem>> {
    let mut by_path: HashMap<String, AvailableModelItem> = HashMap::new();
    for model_dir in model_dirs {
        if !model_dir.is_dir() {
            continue;
        }
        let rd = match fs::read_dir(model_dir) {
            Ok(r) => r,
            Err(e) => {
                tracing::warn!("model dir unreadable {}: {e}", model_dir.display());
                continue;
            }
        };
        for entry in rd {
            ...
            by_path.insert(
                ...
                AvailableModelItem {
                    name,
                    selected: paths_equal_case_insensitive(&full, active_path),
                    path: full,
                },
            );
        }
...
fn paths_equal_case_insensitive(a: &str, b: &str) -> bool {
    if cfg!(windows) {
...
async fn models_handler(State(state): State<Arc<AppState>>) -> Response {
    ...
    match collect_gguf_models(&state.model_dirs, &active_path) {
...
async fn model_switch_handler(
    ...
    if !model_dirs::is_model_path_allowed(&next_model_path, &state.model_dirs) {
        push_gui_event(
            &state,
            "error",
            "model switch rejected: path must be a .gguf under configured model dir(s)",
        );
        return (
            StatusCode::FORBIDDEN,
            Json(serde_json::json!({ "error": "model path not under allowed model directories" })),
        )
            .into_response();
    }
