...
| 2026-07-15 | Qwen3.6-35B-A3B-Uncensored-HauhauCS-Aggressive-Q4_K_M Q4K | AMD Ryzen 5 4500 6-Core Processor               31GB | 19.7 GB | 0.0 GB | 0.0 GB | 1.4 | 4.2 | 3.1x |

The 2026-07-15 row is the Qwen3.6 HauhauCS smoke record documented in the root [README](../README.md). It used `--turboquant-mode exact` because this GGUF has no TurboQuant schema metadata. Runtime placement was CPU-only for model weights because the 19.7 GB file exceeded the available RTX 5060 Ti budget; the measured Hypura `four-tier` + `auto` result was 4.2 decode tok/s versus 1.4 tok/s for the baseline. In practical terms, `legacy-3tier` is borderline usable for a single local text session with short context and modest replies at 5.3 decode tok/s, while the `mmproj` vision request took about 166.5 seconds and is not recommended for interactive use on this machine.
