# C: drive incremental build: CARGO_TARGET_DIR = %LOCALAPPDATA%\hypura-cargo-target
# Default: RUSTFLAGS=-D warnings (zero compiler warnings incl. type-related). Use -AllowWarnings to opt out.
# CARGO_INCREMENTAL=1 + fixed target dir on C: => fast incremental (differential) rebuilds.
# Before overwriting Hypura.exe / hypura.exe, stops running Hypura processes (unless -NoKill).
#
# Usage (repo root):
#   .\scripts\build-hypura-c.ps1
#   .\scripts\build-hypura-c.ps1 -InstallTo "C:\Tools\Hypura"
#   .\scripts\build-hypura-c.ps1 -InstallTo "C:\A","D:\B" -AlsoCopyTo "C:\Legacy"
#   .\scripts\build-hypura-c.ps1 -NoDefaultFeatures -AllowWarnings
#   .\scripts\build-hypura-c.ps1 -PasteCommands          # print Copy-Item lines for manual paste elsewhere
#   .\scripts\build-hypura-c.ps1 -Clipboard            # copy paste block to clipboard (Windows)
#   .\scripts\build-hypura-c.ps1 -CargoOverride "D:\tools\cargo.exe"   # Code Runner 等で cargo だけ別パス
#   .\scripts\build-hypura-c.ps1 -StopOtherCargo   # end other cargo.exe before build (artifact lock)
# Env: HYPURA_CARGO_EXE or HYPURA_CARGO — full path to cargo.exe
...
# VS Code "Code Runner" / some hosts start with a minimal PATH (no User/Machine PATH).
...
function Get-CargoExe {
    ...
    # Non-ASCII in this block breaks PS 5.1 when the file is UTF-8 without BOM (mis-decoded strings).
    $cargoErr = [string]::Join(
        ...
        [string[]]@(
            ...
            '3. Or run: .\scripts\build-hypura-c.ps1 -CargoOverride C:\path\to\cargo.exe',
...
$targetDir = Join-Path $env:LOCALAPPDATA "hypura-cargo-target"
if ($env:LOCALAPPDATA -notmatch '^[A-Za-z]:') {
    throw "LOCALAPPDATA must be on a drive letter path: $env:LOCALAPPDATA"
}
$free = Get-DriveFreeGiB $env:LOCALAPPDATA
if ($free -lt $MinFreeGiB) {
    throw "C: free space is ${free} GiB (need >= ${MinFreeGiB} GiB). Free disk or lower -MinFreeGiB."
}
Write-Host "Using CARGO_TARGET_DIR=$targetDir (free ~${free} GiB on system drive)"
